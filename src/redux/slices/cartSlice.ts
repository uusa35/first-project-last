...
export const cartSlice = createSlice({
  ...
  reducers: {
    setMembership: (state, action: PayloadAction<{ membership: Membership, country: Country, lang: Locale['lang'] }>) => {
      ...
      const redirectUrl = process.env.NODE_ENV === "production" ? `https://cp.ar-expo.ru/redirect/redirect/${lang}?reference_id=${transactionId}` : `http://ar-expo-backend.test/redirect/order/${lang}?reference_id=${transactionId}`;
